...
#### **1. OpenAPI YAML File Not Found**
**Symptom**: `ENOENT: no such file or directory, open '/app/backend/dist/rest/v1/openapi.yaml'`

**Solution**: Ensure volume mounts don't override built files. The Dockerfile copies REST files after build:
```dockerfile
RUN cp -r backend/src/rest backend/dist/
```

#### **2. Environment Variable Parsing Errors**
**Symptom**: `Invalid URL` or quoted values in environment variables

**Solution**: Remove quotes from environment file values:
```bash
# ❌ Wrong
REDIS_URL="redis://redis:6379"
...
#### **3. Prisma/OpenSSL Compatibility**
...
**Symptom**: `ENOTFOUND redis` or `ECONNREFUSED`

**Solutions**:
- Ensure Redis service is healthy: `docker-compose ps`
- Check network connectivity between containers
- Verify environment variables use correct hostnames
